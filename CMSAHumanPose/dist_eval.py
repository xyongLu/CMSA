...
def get_args():
    ...
    parser.add_argument('--batch-size-test', default=1, type=int,  help='Batch size for testing.')
...
def main(args):
    ...
    if args.distributed:
        ...
        sampler_val = torch.utils.data.SequentialSampler(dataset_val)
        print('note: args.distribute==false')
    ...
    if args.model == 'CMSAFormer_L_32' or args.model == 'CMSAFormer_B_32' or args.model == 'CMSAFormer_S_32' :
        model = cmasformer.reparameterize_model(model_without_ddp)

    flops, params = get_model_complexity_info(model, (3, cfg.MODEL.IMAGE_SIZE[1], cfg.MODEL.IMAGE_SIZE[0]), as_strings=True, print_per_layer_stat=False)
